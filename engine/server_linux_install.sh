...
print_usage() {
  cat <<'USAGE'
PrismoVPN Server Linux Installer

Usage:
  bash <(curl -Ls https://raw.githubusercontent.com/prismo/PrismoVPN/main/server_linux_install.sh) [OPTIONS]

Options:
  -v, --version <VERSION>   Install a specific PrismoVPN release (tag), e.g. v1.2.3.
                            If omitted, the latest release is installed.
  -u, --uninstall           Uninstall PrismoVPN: stop and remove the systemd
...
Examples:
  ...
  bash <(curl -Ls https://raw.githubusercontent.com/prismo/PrismoVPN/main/server_linux_install.sh)

  # Install a specific release version:
  bash <(curl -Ls https://raw.githubusercontent.com/prismo/PrismoVPN/main/server_linux_install.sh) --version v2026.04.12.234117-978faee

  # Uninstall PrismoVPN:
  bash <(curl -Ls https://raw.githubusercontent.com/prismo/PrismoVPN/main/server_linux_install.sh) --uninstall
USAGE
}
...
select_release_artifact() {
  ...
  local base_url
  if [[ -n "$version" ]]; then
    base_url="https://github.com/prismo/PrismoVPN/releases/download/${version}"
    log_info "Targeting PrismoVPN release: ${version}"
  else
    base_url="https://github.com/prismo/PrismoVPN/releases/latest/download"
  fi

  case "$arch" in
    aarch64|arm64)
      if [[ $legacy -eq 1 ]]; then
        PREFIX="PrismoVPN_Server_Linux-Legacy_ARM64"
      else
        PREFIX="PrismoVPN_Server_Linux_ARM64"
      fi
      ;;
    armv7l|armv7|armhf)
      PREFIX="PrismoVPN_Server_Linux_ARMV7"
      ;;
    x86_64|amd64)
      if [[ $legacy -eq 1 ]]; then
        PREFIX="PrismoVPN_Server_Linux-Legacy_AMD64"
      else
        PREFIX="PrismoVPN_Server_Linux_AMD64"
      fi
      ;;
    i386|i486|i586|i686|x86)
      PREFIX="PrismoVPN_Server_Linux_X86"
      ;;
    *)
...
if [[ "$ACTION" == "uninstall" ]]; then
  echo -e "          PrismoVPN Server Auto-Uninstaller${NC}"
else
  echo -e "           PrismoVPN Server Auto-Installer${NC}"
fi
echo -e "${CYAN}------------------------------------------------------${NC}"
...
do_uninstall() {
  log_header "Uninstalling PrismoVPN"

  if systemctl list-unit-files --all 2>/dev/null | grep -q '^prismovpn\.service'; then
    log_info "Stopping and disabling prismovpn service..."
    systemctl stop prismovpn 2>/dev/null || true
    systemctl disable prismovpn >/dev/null 2>&1 || true
    systemctl reset-failed prismovpn 2>/dev/null || true
  else
    log_info "No prismovpn systemd unit found."
  fi

  if [[ -f /etc/systemd/system/prismovpn.service ]]; then
    rm -f /etc/systemd/system/prismovpn.service
    log_success "Removed /etc/systemd/system/prismovpn.service"
  fi
  ...
  while IFS= read -r pid; do
    [[ -z "$pid" ]] && continue
    cmdline="$(ps -p "$pid" -o cmd= 2>/dev/null || true)"
    if echo "$cmdline" | grep -qiE 'prismovpn'; then
      log_warn "Terminating stray PrismoVPN process (PID: $pid)..."
      kill "$pid" 2>/dev/null || true
      sleep 1
      if kill -0 "$pid" 2>/dev/null; then
        kill -9 "$pid" 2>/dev/null || true
  ...
  done < <(pgrep -fi 'prismovpn' 2>/dev/null || true)

  if [[ -f /etc/sysctl.d/99-prismovpn.conf ]]; then
    rm -f /etc/sysctl.d/99-prismovpn.conf
    sysctl --system >/dev/null 2>&1 || true
    log_success "Removed kernel tuning (/etc/sysctl.d/99-prismovpn.conf)."
  fi
  if [[ -f /etc/security/limits.d/99-prismovpn.conf ]]; then
    rm -f /etc/security/limits.d/99-prismovpn.conf
    log_success "Removed file descriptor limits (/etc/security/limits.d/99-prismovpn.conf)."
  fi
  ...
  local removed=0
  for f in \
    "$INSTALL_DIR"/PrismoVPN_Server_Linux*_v* \
  ...
  if [[ $removed -eq 0 ]]; then
    log_warn "No PrismoVPN files found in $INSTALL_DIR. If you installed elsewhere, run the uninstaller from that directory."
  fi

  echo -e "\n${CYAN}======================================================${NC}"
  echo -e " ${GREEN}${BOLD}        PRISMOVPN UNINSTALL COMPLETED${NC}"
  echo -e "${CYAN}======================================================${NC}"
...
stop_existing_prismovpn_service() {
  local unit_present=0
  if systemctl list-unit-files --all 2>/dev/null | grep -q '^prismovpn\.service'; then
    unit_present=1
    log_info "Stopping existing PrismoVPN service..."
    systemctl stop prismovpn 2>/dev/null || true

    for _ in 1 2 3 4 5; do
      if ! systemctl is-active --quiet prismovpn; then
        break
      fi
      sleep 1
    done
    ...
    local main_pid
    main_pid="$(systemctl show prismovpn --property MainPID --value 2>/dev/null || true)"
    if [[ -n "${main_pid:-}" && "$main_pid" != "0" ]] && kill -0 "$main_pid" 2>/dev/null; then
      log_warn "prismovpn service is still active. Trying to terminate MainPID: $main_pid"
  ...
  while IFS= read -r pid; do
    [[ -z "$pid" ]] && continue
    cmdline="$(ps -p "$pid" -o cmd= 2>/dev/null || true)"
    if echo "$cmdline" | grep -qiE 'prismovpn|prismovpn_server'; then
      if [[ $killed -eq 0 && $unit_present -eq 0 ]]; then
        log_info "Stopping existing PrismoVPN process that was started outside systemd..."
      fi
...
log_header "Stopping Existing PrismoVPN"
stop_existing_prismovpn_service
...
cat > /etc/sysctl.d/99-prismovpn.conf <<'EOF'
# PrismoVPN high-load tuning
fs.file-max = 2097152
fs.nr_open = 2097152
...
cat > /etc/security/limits.d/99-prismovpn.conf <<'EOF'
* soft nofile 1048576
* hard nofile 1048576
...
if ! DOWNLOAD_DIR="$(mktemp -d /tmp/prismovpn_download.XXXXXX 2>/dev/null)"; then
  DOWNLOAD_DIR="$(mktemp -d "$INSTALL_DIR/prismovpn_download.XXXXXX" 2>/dev/null || true)"
fi
...
SVC="/etc/systemd/system/prismovpn.service"
cat > "$SVC" <<EOF
[Unit]
Description=PrismoVPN Server
...
systemctl enable prismovpn >/dev/null 2>&1
systemctl restart prismovpn

if ! systemctl is-active --quiet prismovpn; then
  journalctl -u prismovpn -n 50 --no-pager || true
  log_error "Service failed to start. See logs above."
fi

log_success "PrismoVPN service is running."

echo -e "\n${CYAN}======================================================${NC}"
echo -e " ${GREEN}${BOLD}       INSTALLATION COMPLETED SUCCESSFULLY!${NC}"
echo -e "${CYAN}======================================================${NC}"
echo -e "${BOLD}Commands:${NC}"
echo -e "  ${YELLOW}>${NC} Start:   systemctl start prismovpn"
echo -e "  ${YELLOW}>${NC} Stop:    systemctl stop prismovpn"
echo -e "  ${YELLOW}>${NC} Restart: systemctl restart prismovpn"
echo -e "  ${YELLOW}>${NC} Logs:    journalctl -u prismovpn -f"
echo -e "\n${BOLD}Files:${NC}"
echo -e "  ${YELLOW}>${NC} ${INSTALL_DIR}/server_config.toml"
echo -e "  ${YELLOW}>${NC} ${INSTALL_DIR}/encrypt_key.txt"
echo -e "${YELLOW}Final Note:${NC} If config changes, run: systemctl restart prismovpn"
