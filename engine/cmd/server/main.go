...
import (
	...
	"prismo-dns-go/internal/runtimepath"
	"prismo-dns-go/internal/security"
	UDPServer "prismo-dns-go/internal/udpserver"
	"prismo-dns-go/internal/version"
)
...
func main() {
	...
	if *versionFlag {
		fmt.Printf("PrismoVPN Server Version: %s\n", version.GetVersion())
		return
	}
	...
	if *logPath != "" {
		log = logger.NewWithFile("PrismoVPN Server", cfg.LogLevel, *logPath)
	} else {
		log = logger.New("PrismoVPN Server", cfg.LogLevel)
	}

	log.Infof("============================================================")
	log.Infof("<cyan>GitHub:</cyan> <yellow>https://github.com/prismo/PrismoVPN</yellow>")
	log.Infof("<cyan>Telegram:</cyan> <yellow>@PrismoVPN</yellow>")
	log.Infof("<cyan>Build Version:</cyan> <yellow>%s</yellow>", version.GetVersion())
	log.Infof("============================================================")

	log.Infof("\U0001F680 <magenta>PrismoVPN Server starting ...</magenta>")
